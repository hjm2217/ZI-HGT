...
# Run this code from the Figures_Code directory
...
res_df <- data.frame()
for (j in 1:18000){
  file_name <- paste0("../Simulations/CARD_Simulations/Results_CARD/Mixnoise", grid$mixnoise[j],
                      "/Sc", grid$scenarios[j], 
...
write.table(Seq.Depth.res, "../../Simulations/Simulation_Results/HGTpCARD_SeqDepthSims_res.txt", row.names=F)

#######################################################
# Prep the CARD with different sequencing depths data #
#######################################################
# We are assuming your CARD simulation results are in a directory Results_CARD

res <- data.frame()
#reslist <- dir("Results_CARD_OG/")

for (i in 1:100){
  tmplist <- readRDS(paste0("../Simulations/CARD_Simulations/Results_CARD/CARD.sim.MOB.res.", i, ".rds")) 
...
# We decided not to use 12 cells, so remove this data
res <- res[res$Ncells < 12,]
write.table(res, "../../Simulations/Simulation_Results/SeqDepthSimsCARDres5.txt", row.names=F)
...
# Plot without the +/- 10% sequencing depth (Figure 4 in main text)
h0 <- ggplot(filter(boxplotdf_0, SeqDepthRelativeToBaselineF == "HGT + CARD"), aes(x = ScenarioF, y = `% Reduction`)) + geom_boxplot(outlier.shape=NA, fill = "green4") + theme_bw() + theme(legend.position="none") +
...
h0
ggsave("../Figures/Figure4.png", h0, units = "in", width = 8)
...
ggsave("../Figures/Supplementary_Figures/FigureS4.png", g0, units = "in", width = 8)
ggsave("../Figures/Supplementary_Figures/FigureS5.png", g1, units = "in", width = 8)
ggsave("../Figures/Supplementary_Figures/FigureS6.png", g2, units = "in", width = 8)
ggsave("../Figures/Supplementary_Figures/FigureS7.png", g3, units = "in", width = 8)
