#########################################################################
## Try denoising the data with MIST then running CARRD as a comparison ##
#########################################################################

# Be sure to conda activate ReST (the MIST conda environment) before running this script
...
# Set the file name to save the output
file_name <- paste0("SPARSim_Results/Sample_2/MIST/MIST_SPARSim_pseudo_OSCC_n", grid$ntotal[j],
                    "_cells_library_factor_", grid$lib_factor[j],
                    "_Phi_factor_", grid$Phi_factor[j],
                    "_scRNAseq_rep_", grid$SC_replicate[j],
                    "_ST_replicate_", grid$ST_replicate[j], ".rds")
...
# Set the file name for the input to MIST
file_name_for_MIST <- paste0("SPARSim_Results/Sample_2/MIST/data_for_MIST/",
                         "MIST_SPARSim_pseudo_OSCC_n", grid$ntotal[j],
                         "_cells_library_factor_", grid$lib_factor[j],
                         "_Phi_factor_", grid$Phi_factor[j],
                         "_scRNAseq_rep_", grid$SC_replicate[j],
                         "_ST_replicate_", grid$ST_replicate[j], ".csv")

# Set the file name for MIST's output
file_name_from_MIST <- paste0("SPARSim_Results/Sample_2/MIST/MIST_output/",
                          "MIST_SPARSim_pseudo_OSCC_n", grid$ntotal[j],
                          "_cells_library_factor_", grid$lib_factor[j],
                          "_Phi_factor_", grid$Phi_factor[j],
                          "_scRNAseq_rep_", grid$SC_replicate[j],
...
sim <- SimOverCARD_OSCC_MIST(sc_count = eset.sub.split2@assays$simSC@counts,
                             ...
                             scenario = 1,
                             seed = grid$iseed[j],
                             file_name_for_MIST = file_name_for_MIST,
                             file_name_from_MIST = file_name_from_MIST)
