###########################################################################
## Try imputing missing data with ALRA then running CARD as a comparison ##
###########################################################################
...
file_name <- paste0("SPARSim_Results/Sample_2/ALRA/ALRA_SPARSim_pseudo_OSCC_n", grid$ntotal[j],
                    "_cells_library_factor_", grid$lib_factor[j],
                    "_Phi_factor_", grid$Phi_factor[j],
                    "_scRNAseq_rep_", grid$SC_replicate[j],
                    "_ST_replicate_", grid$ST_replicate[j], ".rds")
...
sim <- SimOverCARD_OSCC_ALRA(sc_count = eset.sub.split2@assays$simSC@counts,
