...
# Set up the grid
lib_factor <- c(0.1, 0.05, 0.03, 0.01)
Phi_factor <- c(10, 20, 50, 100)
SC_rep <- 1:10
grid <- expand.grid(lib_factor, Phi_factor, SC_rep)
colnames(grid) <- c("lib_factor", "Phi_factor", "SC_rep")
grid$seed <- rep(2000 * 1:16, 10) 

# remove the rows with lib_factors we didn't wind up using (we do this to maintain same seeds)
grid <- grid[grid$lib_factor %in% c(0.1,0.05),]
