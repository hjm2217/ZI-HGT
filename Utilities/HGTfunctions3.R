...
HGTplusCARD <- function(sc_count,
  ...
  #Initialize the list of transformations and log-likelihoods
  TcellTypePropMatList <- list()
  logliklist <- list()
  ...
  if (doWAIC){
    ...
    gc()
    cat("done with transformed run", i, "\n")
    cat("dimensions of cell type prop mat", dim(TcellTypePropMatList[[i]]), "\n")
  }
  ...
  lowerMat <- apply(medianArray, 1:2, quantile, 0.025)
  rownames(lowerMat) <- rownames(cellTypePropMatAve)
  colnames(lowerMat) <- colnames(cellTypePropMatAve)
  upperMat <- apply(medianArray, 1:2, quantile, 0.975)
  rownames(upperMat) <-	rownames(cellTypePropMatAve)
  colnames(upperMat) <-	colnames(cellTypePropMatAve)

  cellTypePropMatList <- list(mean_ct_prop = cellTypePropMatAve, median_ct_prop = cellTypePropMatMedian, lower_95_prop = lowerMat, upper_95_prop = upperMat, transformed_ct_prop_mat_list = TcellTypePropMatList)
...
HGT.CARD.two.tone <- function(proportion, spatial_location, ct.visualize = ct.visualize, colors = c("grey70","darkgreen"), NumCols, pointSize = 3.0, cutoff = 0.05){
  ...
  p = suppressMessages(ggplot(mData, aes(x, y)) +
                         ...
                         scale_x_discrete(expand = c(0, 1)) + scale_y_discrete(expand = c(0,1))+
                         facet_wrap(~Cell_Type,ncol = NumCols, labeller = labeller(Cell_Type = label_wrap_gen(20))) +
                         coord_fixed() + labs(colour = "Proportion > 0.05") + 
                         theme(plot.margin = margin(0.1, 0.1, 0.1, 0.1, "cm"),
                               #legend.position=c(0.14,0.76),
                               panel.background = element_blank(),
                               plot.background = element_blank(), #element_blank(),
